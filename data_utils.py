...
"""My Own version"""
class TextAudioMyOwnLoader(torch.utils.data.Dataset):
    """
        1) loads audio, speaker_id, text pairs
        2) normalizes text and converts them to sequences of integers
        3) computes spectrograms from audio files.
    ...
    def __init__(self, audiopaths_sid_text, hparams):
        self.audiopaths_sid_text = load_filepaths_and_text(audiopaths_sid_text)
        print(len(self.audiopaths_sid_text))
        self.text_cleaners = hparams.text_cleaners
        self.max_wav_value = hparams.max_wav_value
        self.sampling_rate = hparams.sampling_rate
        self.filter_length  = hparams.filter_length
    ...
    def _filter(self):
        ...
        audiopaths_sid_text_new = []
        lengths = []
        for audiopath, lid, text in self.audiopaths_sid_text:
            if self.min_text_len <= len(text) and len(text) <= self.max_text_len:
                audiopaths_sid_text_new.append([audiopath, lid, text])
                lengths.append(os.path.getsize(audiopath) // (2 * self.hop_length))
        self.audiopaths_sid_text = audiopaths_sid_text_new
        self.lengths = lengths

    def get_audio_text_speaker_pair(self, audiopath_lid_text):
        # separate filename, speaker_id and text
        ...
        return (text, spec, wav, spk_emb, emo_emb, lid)
...
class TextAudioMyOwnCollate():
    ...
    def __call__(self, batch):
        ...
        lids = torch.LongTensor(len(batch))
        ...
        if self.return_ids:
            return text_padded, text_lengths, spec_padded, spec_lengths, wav_padded, wav_lengths, spk_embeds, emo_embeds, lids, ids_sorted_decreasing
        return text_padded, text_lengths, spec_padded, spec_lengths, wav_padded, wav_lengths, spk_embeds, emo_embeds, lids
